const { DEFAULT_STATE, ALLOWED_FORMATS, ALLOWED_TEMPLATES } = require('../constants/defaults')
...
function verifyState(state, colorValidFunc = isValidColor, textValidFunc = isValidText) {
  ...
  // these will need to change as well.
  ...
  if (ALLOWED_FORMATS.indexOf(state.format) === -1) {
    throw new Error(`Invalid format: ${state.format}`)
  }
  if (ALLOWED_TEMPLATES.indexOf(state.template) === -1) {
    throw new Error(`Invalid template: ${state.template}`)
  }
