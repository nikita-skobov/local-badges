...
const { DEFAULT_STATE, FUNCTION_KEY, FOLDER_KEY, FILE_KEY } = require('./constants/defaults')
...
describe('badgeRunner module', () => {
  ...
  describe('badgeRunner function', () => {
    ...
    it('should call the makeFolderFunc with the badge', () => {
      const badgeObj = {
        [FILE_KEY]: 'some-file',
        [FOLDER_KEY]: 'some-folder',
      }
      ...
      const makeFolderMock = jest.fn()
      badgeRunner(configObj7, cliObj, undefined, undefined, undefined, makeFolderMock)
      expect(makeFolderMock).toHaveBeenCalledWith(badgeObj)
    })
  })
})
